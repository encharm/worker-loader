...
${esModule ? "export default" : "module.exports ="} function ${fnName}() {\n  return worker(${JSON.stringify(workerSource)}, ${JSON.stringify(workerConstructor)}, ${JSON.stringify(workerOptions)}, ${fallbackWorkerPath});\n}\n`;
  ...
  const str = `${loaderContext.target === 'node' ? esModule ? "import {Worker} from 'worker_threads'\n" : "var {Worker} = require('worker_threads')\n" : ''}${esModule ? "export default" : "module.exports ="} function ${fnName}(workerData) {\n  return new ${workerConstructor}(${publicPath} + ${JSON.stringify(workerFilename)}${workerOptions ? `, ${JSON.stringify(workerOptions)}` : ",{workerData}"});\n}\n`;
  ;
